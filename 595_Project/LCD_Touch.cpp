...
/*******************************************************************************
  function:
        Makes a button at the designated area, change pages
*******************************************************************************/
void MakeButton(POINT x1, POINT x2, POINT y1, POINT y2, char* page, char value, int* startAdjusting, int* charging, uint16_t* tempSlider, uint16_t* brightSlider) {
  if(sTP_Draw.Xpoint > x1 && sTP_Draw.Xpoint < x2 && sTP_Draw.Ypoint > y1 && sTP_Draw.Ypoint < y2) {
    *page = value;      
...
/*******************************************************************************
  function:
        Show which charging method is selected
*******************************************************************************/
void ChargingMethod(int* charging) {
    if(sTP_Draw.Xpoint > 20 && sTP_Draw.Xpoint < 230 && sTP_Draw.Ypoint > 180 && sTP_Draw.Ypoint < 300 && (*charging == 1)) { // Battery
      *charging = 0;
      DrawSelectionBoxCharging(charging);
    }
    else if(sTP_Draw.Xpoint > 250 && sTP_Draw.Xpoint < 460 && sTP_Draw.Ypoint > 180 && sTP_Draw.Ypoint < 300 && (*charging == 0)) { // Solar
      *charging = 1;
      DrawSelectionBoxCharging(charging);
    } 
}
...
/*******************************************************************************
  function:
        Sends signal to Thermoelectric Power Switch and Fan Driver
*******************************************************************************/
void StartAdjusting(int* startAdjusting, float* readTemp, uint16_t* userTemp) {
    if(sTP_Draw.Xpoint > 280 && sTP_Draw.Xpoint < 360 && sTP_Draw.Ypoint > 20 && sTP_Draw.Ypoint < 60 && (*startAdjusting == 0)) {
      *startAdjusting = 1;
      DrawSelectionBoxTemp(startAdjusting);
    }
    else if(sTP_Draw.Xpoint > 380 && sTP_Draw.Xpoint < 460 && sTP_Draw.Ypoint > 20 && sTP_Draw.Ypoint < 60 && (*startAdjusting == 1)) {
      *startAdjusting = 0;
      DrawSelectionBoxTemp(startAdjusting);
    } 
}
...
/*******************************************************************************
  function:
        Convert the temp slider to a value in degrees Celcius
*******************************************************************************/
void convertCelcius(uint16_t* slider, uint16_t* actual) {
  int counter = 1;
  int compare = 11;
  int sliderTemp = *slider - 30;

  while(counter < 37) {
    if(sliderTemp <= compare) { // If the slider pixel value is less than or equal to the compare pixel value
      *actual = (uint16_t)(counter + 1);
...
/*******************************************************************************
  function:
        Convert the temp slider to a value in degrees Celcius for a comparison
*******************************************************************************/
int checkCelcius(uint16_t* slider) {
  int counter = 1;
  int compare = 11;
  int sliderTemp = *slider - 30;
...
/*******************************************************************************
  function:
        Make a slider on the given page
*******************************************************************************/
void MakeTempSlider(uint16_t* sliderPosition, uint16_t* userTemp) {
  if(sTP_Draw.Xpoint > 30 && sTP_Draw.Xpoint < 450 && sTP_Draw.Ypoint > 200 && sTP_Draw.Ypoint < 290) {
     if(checkCelcius(sliderPosition) >= (checkCelcius(sTP_Draw.Xpoint) + 2)) { // User wants to cool
       digitalWrite(Direction, HIGH);
     }
     else if(checkCelcius(sliderPosition) <= (checkCelcius(sTP_Draw.Xpoint) - 2)) { // User wants to heat
       digitalWrite(Direction, LOW);
     }
     *sliderPosition = sTP_Draw.Xpoint;
     convertCelcius(sliderPosition, userTemp);
     DrawTempSlider(sliderPosition);
  }
}
...
/*******************************************************************************
  function:
        Make a slider on the given page
*******************************************************************************/
void MakeBrightSlider(uint16_t* sliderPosition) {
  if(sTP_Draw.Xpoint > 30 && sTP_Draw.Xpoint < 450 && sTP_Draw.Ypoint > 200 && sTP_Draw.Ypoint < 290) {
     *sliderPosition = sTP_Draw.Xpoint;
     double temp1 = floor((double(*sliderPosition) - 30) * 255 / 420);
     PWM_SetValue(uint16_t(temp1));
     DrawBrightSlider(sliderPosition);
  }
}
...
*******************************************************************************/
void Cursor(long* timer, int* sleep, char* page, int* startAdjusting, int* charging, uint16_t* tempSlider, uint16_t* brightSlider, float* readTemp, uint16_t* userTemp) {
  TP_Scan(0);
  
  if (sTP_DEV.chStatus & TP_PRESS_DOWN) { // If user pressed anywhere on the screen

    // Checks to see where user pressed
    if(*sleep != 2) { // If screen isn't in the 2nd stage of sleep
      if(*page == '0') { // Home
        MakeButton(20,220,100,180,page,'1',startAdjusting,charging,tempSlider,brightSlider);
        MakeButton(260,460,100,180,page,'2',startAdjusting,charging,tempSlider,brightSlider);
        MakeButton(20,220,220,300,page,'3',startAdjusting,charging,tempSlider,brightSlider);
        MakeButton(260,460,220,300,page,'4',startAdjusting,charging,tempSlider,brightSlider);
      }
      else if(*page == '1') { // Adjust Temperature 
        MakeTempSlider(tempSlider, userTemp);
        MakeButton(20,60,20,60,page,'0',startAdjusting,charging,tempSlider,brightSlider);
        StartAdjusting(startAdjusting, readTemp, userTemp);
      }
    ...
    // Brings system out of sleep mode
    if(*timer != 0) {
      *timer = 0;
      *sleep = 0;
      PWM_SetValue(*brightSlider);
      delay(1000); // Delay 1 second to allow user time to stop touching screen
    }
  }
}
